...
2. Create a variable that holds the current value at each step of the factorial. In my solution below, this is ```factorial```. In the example above, this variable will hold the value of 1 * 2 (2). After being multiplied by 3, it will store the new value (6). Afer being multiplied by 4, store the product once again
...
Note: You need to start with 1, not 0, because the result of the factorializing process must equal at least 1. (If you want an explanation of why 0! = 1, [read this](http://mathforum.org/library/drmath/view/57128.html)).
